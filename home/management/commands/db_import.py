...
class Command(BaseCommand):
    help = 'Import questions for the book Nehemiah from CSV'

    def handle(self, *args, **kwargs):
        csv_file_path = 'nehemiah_questions.csv'

        category, _ = Categories.objects.get_or_create(name="Old Testament")
        book, _ = Books.objects.get_or_create(name="Nehemiah", category=category)

        def extract_chapter_verse(text):
            match = re.search(r'\(Neh\s+(\d+):(\d+)\)', text)
            if match:
        ...
        self.stdout.write(self.style.SUCCESS("✅ Nehemiah questions imported successfully."))
