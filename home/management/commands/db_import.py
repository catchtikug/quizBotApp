...
class Command(BaseCommand):
    help = 'Import questions for the book of Hosea from CSV'

    def handle(self, *args, **kwargs):
        csv_file_path = 'hosea_questions.csv'

        category, _ = Categories.objects.get_or_create(name="Old Testament")
        book, _ = Books.objects.get_or_create(name="Hosea", category=category)

        def extract_chapter_verse(text):
            match = re.search(r'\(Hos\s+(\d+):(\d+)\)', text)
            if match:
        ...
        self.stdout.write(self.style.SUCCESS("✅ Hosea questions imported successfully."))
