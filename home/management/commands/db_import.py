...
class Command(BaseCommand):
    help = 'Import questions for the book Esther from CSV'

    def handle(self, *args, **kwargs):
        csv_file_path = 'esther_questions.csv'

        category, _ = Categories.objects.get_or_create(name="Old Testament")
        book, _ = Books.objects.get_or_create(name="Esther", category=category)

        def extract_chapter_verse(text):
            match = re.search(r'\(Est\s+(\d+):(\d+)\)', text)
            if match:
        ...
        self.stdout.write(self.style.SUCCESS("✅ Esther questions imported successfully."))
