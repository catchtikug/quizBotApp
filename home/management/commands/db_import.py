...
class Command(BaseCommand):
    help = 'Import questions for the book of Leviticus from CSV'

    def handle(self, *args, **kwargs):
        csv_file_path = 'leviticus_questions.csv'

        category, _ = Categories.objects.get_or_create(name="Old Testament")
        book, _ = Books.objects.get_or_create(name="Leviticus", category=category)

        def extract_chapter_verse(text):
            match = re.search(r'\(Lev\s+(\d+):(\d+)\)', text)
            if match:
        ...
        self.stdout.write(self.style.SUCCESS("✅ Leviticus questions imported successfully."))
