...
class Command(BaseCommand):
    help = 'Import questions for the book of Mark from CSV'

    def handle(self, *args, **kwargs):
        csv_file_path = 'mark_questions.csv'

        category, _ = Categories.objects.get_or_create(name="Old Testament")
        book, _ = Books.objects.get_or_create(name="Mark", category=category)

        def extract_chapter_verse(text):
            match = re.search(r'\(Mk\s+(\d+):(\d+)\)', text)
            if match:
        ...
        self.stdout.write(self.style.SUCCESS("✅ Mark questions imported successfully."))
