...
class Command(BaseCommand):
    help = 'Import questions for the book of Genesis from CSV'

    def handle(self, *args, **kwargs):
        csv_file_path = 'genesis_questions.csv'

        category, _ = Categories.objects.get_or_create(name="Old Testament")
        book, _ = Books.objects.get_or_create(name="Genesis", category=category)

        def extract_chapter_verse(text):
            match = re.search(r'\(Gen\s+(\d+):(\d+)\)', text)
            if match:
        ...
        self.stdout.write(self.style.SUCCESS("✅ Genesis questions imported successfully."))
